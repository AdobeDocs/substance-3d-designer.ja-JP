...
+++パラメーターとは何ですか？
*パラメーターは、グラフの動作を制御するUI要素を持つ単純な値です。*&#x200B;色を変える、描画モードを設定する、不透明度の値を選ぶなど、すべてのSubstanceソフトウェアで常に使用します。パラメーターがなければ、Substanceソフトウェアではどのようなカスタマイズも行えません。
...
1. <b>バッチ公開パラメーターの選択…ドロップダウンメニューの</b>
...
1. 既に公開されているパラメーターの横にあるドロップダウンオプションボタン![](../../../assets/image2020-9-17-15-35-59.png)をクリックします
1. ![](../../../assets/image2020-9-17-15-37-7.png)<b>公開されたグラフ入力を編集</b>を選択します。 これにより、グラフプロパティの関連するエントリに直接アクセスできます
1. グラフの空の領域をダブルクリックしてグラフのプロパティを表示し、<b>入力パラメーター</b>の一覧でパラメーターを見つけます
1. <b>エクスプローラー</b>でグラフを1回クリックし、<b>入力パラメーター</b>の一覧でパラメーターを見つけます
