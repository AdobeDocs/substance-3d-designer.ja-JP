...
![単純化されたパラメーターを公開しています](../../../assets/parameters-5.png "単純化されたパラメーターを公開しています")
...
![単一の公開メソッドのウォークスルー](../../../assets/single-expose2.gif "単一の公開メソッドのウォークスルー"){width="512px"}
...
1. <b>[OK]</b>を押して確定します
1. パラメーターの名前が&#x200B;*青*、および![](../../../assets/image2020-9-17-15-35-46.png)に変わります\
...
1. 単一のパラメーターを検索する代わりに、<b>特定のパラメーター</b>タブの右上にある![](../../../assets/image2020-9-17-15-39-7.png) <b>複数公開</b>ボタンを見つけます
...
1. 選択したすべてのパラメーターを確認して表示するには、<b>[OK]</b>をクリックします。 パラメーター名に&#x200B;*青*&#x200B;が表示され、パラメーターが公開されたことが確認され、![](../../../assets/image2020-9-17-15-35-46.png) <b>関数の編集</b>ボタンが表示されます。
...
表示されるパラメーターのオプションを変更するには、次のいずれかの操作を行います。

1. 既に表示されるパラメーターされている項目の横にあるドロップダウンオプションボタン![](../../../assets/image2020-9-17-15-35-59.png)をクリックします
1. ![](../../../assets/image2020-9-17-15-37-7.png)<b> [グラフ入力の編集]</b>を選択します。 これにより、グラフプロパティの関連するエントリに直接移動します
...
![入力パラメーター](../../../assets/input-parameters-2.png "入力パラメーター"){width="512px"}

### 入力パラメーター

公開されているパラメーターはすべて、「入力パラメーター」タブに一覧表示されます。 次のプロパティは、浮動小数や整数など、デフォルトのエディタータイプを持つほとんどの一般的なケースで使用できます。

1. <b>識別子</b>：このパラメーターの一意の識別子です。 スペースまたは特殊文字を含めることはできません。
1. <b>ラベル</b>: UIのみのラベル。 ラベルが定義されていない場合は、UIに識別子が表示されます。 スペースと特殊文字を含めることができます
...
![整数パラメーターのドロップダウンリストエディター](../../../assets/dropdown.gif "整数パラメーターのドロップダウンリストエディター"){width="512px"}
...
![入力パラメーターの並べ替え](../../../assets/reorder-2.gif "入力パラメーターの並べ替え"){width="512px"}

#### 再注文

パラメータの並べ替えは、リストの入力パラメータ名の左側にある暗いストライプのハンドルをドラッグ&amp;ドロップすることで簡単に行うことができます。 パラメータをグループ化すると順序に影響する場合があることに注意してください。

![入力パラメーターのプレビュー](../../../assets/parameter-preview-2.gif "入力パラメーターのプレビュー"){width="512px"}
...
[コピー]ボタン![](../../../assets/image2019-9-19-11-3-49.png)を使用して、単一のパラメーターをコピーできます。 パラメーターメニュー![](../../../assets/image2020-9-17-15-39-7.png)を使用して、複数のパラメーターをコピーできます。 「入力をコピー」を選択して、すべての入力をコピーします。

パラメーターメニュー![](../../../assets/image2020-9-17-15-39-7.png)の[入力![](../../../assets/image2020-9-17-16-43-15.png)の貼り付け]を選択して、1つ以上のパラメーターを貼り付けます。
...
パラメータの性質上、ノード上で複数のノードを制御できる場合、または入力パラメーターを制御せずに入力パラメーターが存在できる場合、パラメータが存在しないか使用されていないことが原因で問題が発生する可能性があります。 以下では、一般的な問題とその解決策について説明します。

![ノードパラメーターのエラー](../../../assets/parameter-error.gif "ノードパラメーターのエラー"){width="512px"}

### ノード上の壊れたパラメータのトラッキング

グラフビューの上部バーにあるノードファインダーツール![](../../../assets/image2019-9-19-14-15-53.png)を使用して、どのノードがどのパラメーターを使用しているかを追跡できます。 クリックすると、特定のパラメータを使用してノードを検索できます。

ノードに実際の問題がある場合は、ノードの左上隅に警告バッジ![](../../../assets/image2019-9-19-14-23-54.png)が表示されます。 バッジの上にマウスポインターを置くと、詳細情報を示すツールヒントが表示されます。

問題をリセットして削除するには、修正またはリセットするパラメーターの[関数の編集]ボタンの横にあるドロップダウンボタン![](../../../assets/image2020-9-17-15-35-59.png)をクリックし、![](../../../assets/image2020-9-17-16-56-18.png) <b>リセットを選択します。 </b>これにより、パラメーターが前の公開されていない状態に戻ります。これを反映して青い名前が再びグレーになります。

![使用されていない入力パラメーターをクリーンアップしています](../../../assets/clean-inputs-2.gif "使用されていない入力パラメーターをクリーンアップしています"){width="512px"}

### 未使用の入力パラメーターをクリーンアップしています

入力パラメーターのトラックが不明で、使用されているパラメーターがわからない場合は、小さなツールを使用してクリーンアップできます。 入力パラメーターメニューボタン![](../../../assets/image2020-9-17-15-39-7.png)をクリックし、<b>入力を消去</b>を選択します。
...
![パラメーターを削除しています](../../../assets/delete-param-2.gif "パラメーターを削除しています"){width="512px"}

### パラメータの削除

使用中のパラメータを実際に削除するには、2つの異なる手順が必要です。

1. パラメーターが表示されているノードで、青色の関数表示ボタンの右側にあるドロップダウン矢印をクリックします。![](../../../assets/image2019-9-19-14-55-55.png) 次に、「デフォルト値にリセット」を選択します。 これにより、この1つのノードでパラメータを使用する必要がなくなります。 同じパラメータを使用する他のノードに対して、この手順を繰り返します。 [既定値にリセット]をクリックすると、パラメーターウィジェットの範囲も&#x200B;*ソフト範囲*&#x200B;にリセットされます。
