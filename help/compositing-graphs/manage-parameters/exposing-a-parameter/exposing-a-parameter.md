...
![単純化されたパラメーターを公開しています](exposing-a-parameter.resources/exposing-a-parameter-01.png "単純化されたパラメーターを公開しています")
...
![単一の公開メソッドのウォークスルー](exposing-a-parameter.resources/exposing-a-parameter-02.gif "単一の公開メソッドのウォークスルー"){width="512px"}

### 単一露光法

1. [プロパティ](../../../interface/properties/properties.md)パネルの[特定のパラメーター]タブで、表示するパラメーターを指定します
1. ![](exposing-a-parameter.resources/exposing-a-parameter-03.png)ドロップダウンオプションボタンをクリックします
1. 最初のオプションであるドロップダウンリストから![](exposing-a-parameter.resources/exposing-a-parameter-04.png) <b>新しいグラフ入力として表示</b>を選択します。
1. <b>パラメーターを公開</b>ダイアログが表示されます。必要に応じてパラメーターのプロパティを設定します。

   <b>識別子</b>と<b>ラベル</b>を少なくとも変更することをお勧めします
1. <b>[OK]</b>を押して確定します
1. パラメーターの名前が&#x200B;*青*、および![](exposing-a-parameter.resources/exposing-a-parameter-05.png)に変わります\
...
![メソッドの一括公開](exposing-a-parameter.resources/exposing-a-parameter-06.gif "メソッドの一括公開"){width="512px"}

### バッチ公開方法

1つのパラメーターを公開する場合、このメソッドは前のメソッドよりも少し遅くなります。 複数のパラメーターを公開する場合は、はるかに高速になります。

1. 単一のパラメーターを検索する代わりに、<b>特定のパラメーター</b>タブの右上にある![](exposing-a-parameter.resources/exposing-a-parameter-07.png) <b>複数公開</b>ボタンを見つけます
1. ドロップダウンメニューから[<b>パラメーターの一括公開…</b>]を選択します
1. <b>バッチ公開</b>ダイアログが表示され、ノードのすべての<b>特定のパラメーター</b>の公開をカスタマイズできます
1. <b>すべて</b>、<b>なし</b>、または特定のチェックボックスを使用して、表示するパラメーターを決定します
1. リストの<b>グラフ入力ID</b>列の下にあるパラメーター名をクリックして、パラメーター名を変更します。
1. リストの<b>グラフ入力グループ</b>列の<b>グループ名</b>をクリックして、1つの特定のパラメーターの（サブ）グループを追加します
1. 下部にある<b>グラフ入力ID</b>と<b>グラフ入力グループ</b>の入力ボックスを使用して、公開されたすべてのパラメーターにプレフィックス、サフィックス、および入力グループを一度に追加します。 これらの値はすべて、パラメーターごとの設定の上に適用されます。
1. 選択したすべてのパラメーターを確認して表示するには、<b>[OK]</b>をクリックします。 パラメーター名に&#x200B;*青*&#x200B;が表示され、パラメーターが公開されたことが確認され、![](exposing-a-parameter.resources/exposing-a-parameter-05.png) <b>関数の編集</b>ボタンが表示されます。

## 制限

次の表に示すように、パラメーターの公開に関連するいくつかの制限があります。
...
1. 既に公開されているパラメーターの横にあるドロップダウンオプションボタン![](exposing-a-parameter.resources/exposing-a-parameter-03.png)をクリックします
1. ![](exposing-a-parameter.resources/exposing-a-parameter-04.png)<b> [グラフ入力の編集]</b>を選択します。 これにより、グラフプロパティの関連するエントリに直接アクセスできます
...
![入力パラメーター](exposing-a-parameter.resources/exposing-a-parameter-08.png "入力パラメーター"){width="512px"}
...
![整数パラメーターのドロップダウンリストエディター](exposing-a-parameter.resources/exposing-a-parameter-09.gif "整数パラメーターのドロップダウンリストエディター"){width="512px"}
...
![入力パラメーターの並べ替え](exposing-a-parameter.resources/exposing-a-parameter-10.gif "入力パラメーターの並べ替え"){width="512px"}

#### 再注文

パラメータの並べ替えは、リストの入力パラメータ名の左側にある暗いストライプのハンドルをドラッグ&amp;ドロップすることで簡単に行うことができます。 パラメータをグループ化すると順序に影響する場合があることに注意してください。

![入力パラメーターのプレビュー](exposing-a-parameter.resources/exposing-a-parameter-11.gif "入力パラメーターのプレビュー"){width="512px"}
...
![入力パラメーターのコピーと貼り付け](exposing-a-parameter.resources/exposing-a-parameter-12.gif "入力パラメーターのコピーと貼り付け"){width="512px"}

### パラメータのコピー&amp;ペースト

パラメータは、グラフ間でコピー&amp;ペーストできます。

[コピー]ボタン![](exposing-a-parameter.resources/exposing-a-parameter-13.png)を使用して、単一のパラメーターをコピーできます。 パラメーターメニュー![](exposing-a-parameter.resources/exposing-a-parameter-07.png)を使用して、複数のパラメーターをコピーできます。 「入力をコピー」を選択して、すべての入力をコピーします。

パラメーターメニュー![](exposing-a-parameter.resources/exposing-a-parameter-07.png)の[入力![](exposing-a-parameter.resources/exposing-a-parameter-14.png)の貼り付け]を選択して、1つ以上のパラメーターを貼り付けます。
...
![ノードパラメーターのエラー](exposing-a-parameter.resources/exposing-a-parameter-15.gif "ノードパラメーターのエラー"){width="512px"}

### ノード上の壊れたパラメータのトラッキング

グラフビューの上部バーにあるノードファインダーツール![](exposing-a-parameter.resources/exposing-a-parameter-16.png)を使用して、どのノードがどのパラメーターを使用しているかを追跡できます。 クリックすると、特定のパラメータを使用してノードを検索できます。

ノードに実際の問題がある場合は、ノードの左上隅に警告バッジ![](exposing-a-parameter.resources/exposing-a-parameter-17.png)が表示されます。 バッジの上にマウスポインターを置くと、詳細情報を示すツールヒントが表示されます。

問題をリセットして削除するには、修正またはリセットするパラメーターの[関数の編集]ボタンの横にあるドロップダウンボタン![](exposing-a-parameter.resources/exposing-a-parameter-03.png)をクリックし、![](exposing-a-parameter.resources/exposing-a-parameter-18.png) <b>リセットを選択します。 </b>これにより、パラメーターが前の公開されていない状態に戻ります。これを反映して青い名前が再びグレーになります。

![使用されていない入力パラメーターをクリーンアップしています](exposing-a-parameter.resources/exposing-a-parameter-19.gif "使用されていない入力パラメーターをクリーンアップしています"){width="512px"}

### 未使用の入力パラメーターをクリーンアップしています

入力パラメーターのトラックが不明で、使用されているパラメーターがわからない場合は、小さなツールを使用してクリーンアップできます。 入力パラメーターメニューボタン![](exposing-a-parameter.resources/exposing-a-parameter-07.png)をクリックし、<b>入力を消去</b>を選択します。
...
![パラメーターを削除しています](exposing-a-parameter.resources/exposing-a-parameter-20.gif "パラメーターを削除しています"){width="512px"}

### パラメータの削除

使用中のパラメータを実際に削除するには、2つの異なる手順が必要です。

1. パラメーターが表示されているノードで、青色の関数表示ボタンの右側にあるドロップダウン矢印をクリックします。![](exposing-a-parameter.resources/exposing-a-parameter-21.png) 次に、「デフォルト値にリセット」を選択します。 これにより、この1つのノードでパラメータを使用する必要がなくなります。 同じパラメータを使用する他のノードに対して、この手順を繰り返します。 [既定値にリセット]をクリックすると、パラメーターウィジェットの範囲も&#x200B;*ソフト範囲*&#x200B;にリセットされます。
