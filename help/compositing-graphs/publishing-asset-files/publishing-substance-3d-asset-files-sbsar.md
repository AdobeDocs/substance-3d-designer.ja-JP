...
* すべての[Bitmap](../../compositing-graphs/nodes-reference-for-com/atomic-nodes/bitmap/bitmap.md)ノードの[Output size](../../compositing-graphs/output-size/output-size.md)プロパティが&#x200B;*Absolute* [継承メソッド](../../compositing-graphs/inheritance-compositing/inheritance-in-substance-compositing-graphs.md)に設定されていることを確認してください。 そうでない場合は、参照されている[ビットマップリソース](../../resources/bitmap-resource/bitmap-resource.md)が、公開されているSubstance 3Dアセットファイルのデフォルトの<b>256\*256</b>解像度で保存されます。これにより、1つ以上の出力の*&#x200B;品質*に影響します。
* Designer以外では使用できないグラフがパッケージ内に存在する場合（例えば、特定のコンテキストでのみ機能するヘルパーまたは「ツール」サブグラフなど）、プロパティで非表示になるように設定します。 詳しくは、以下を参照してください。

## 公開方法

公開の準備ができたら、[エクスプローラー](../../interface/the-explorer-window/the-explorer-window.md)から公開ダイアログにアクセスできます。
...
「アセットのPublish」オプションが表示される前に、Substance 3Dファイル(SBS)が保存されていない場合は保存するように求められ、Substance 3Dアセットの保存先を確認するメッセージが表示されます。 ファイルに関するメッセージやダイアログが表示され、ファイルの送信が速くなるのを防ぐには、前述の<b>Publishを以前の</b>と同じ方法で使用します。
...
<b>見つからないアイコンを生成する</b>では、組み込みの[PBR レンダリング](../../compositing-graphs/nodes-reference-for-com/node-library/material-filters/pbr-utilities/pbr-render/pbr-render.md)手法を使用して、各グラフの属性のサムネイルを作成します。

<b>グラフ </b>このパッケージで表示されるすべてのグラフの一覧を表示します。グラフの除外については、以下を参照してください。

>[!NOTE]
>
> **ランダムシードの露出**
> 
> ランダムシードの露光量設定は、Publishダイアログで使用できなくなりました。 代わりに、[グラフのランダムシード属性を相対値ではなく絶対値に設定して、使用できないようにします。](../../compositing-graphs/graph-parameters/graph-parameters.md)

## 公開されたアセットからのグラフの除外

パッケージ内の一部のグラフは、外部での使用を意図していない場合があります。 これらのサブグラフは、通常、より大きな全体の一部として、つまりマスターマテリアルのサブルーチンとして使用されます。

<table>
<tr style="border: 0;">
<td style="border: 0;" valign="top">

Substance 3D グラフファイル内でアセットが表示または使用可能にならないようにするには、そのグラフのプロパティにアクセスし（グラフビュー内の空の領域をダブルクリック、またはエクスプローラー内のグラフをシングルクリック）、<b>属性</b>ロールアウトを開きます。 <b>SBSAR</b>で表示<b>いいえ</b>に設定して、公開時に非表示にします。
...
* 1つ以上のグラフに出力がありません\
  この警告は、出力ノードを持たない1つ以上のグラフを含むパッケージを公開しようとしていることを示します。 解決方法は、黄色の警告トライアングルを使用して[出力ノード](../../compositing-graphs/nodes-reference-for-com/atomic-nodes/output/output.md)をグラフに追加することです。
* 1 つ以上のグラフに親に相対的でない出力サイズのパラメーターがあります\
  1つ以上のグラフの出力サイズが正しく設定されていません。 通常は、グラフ自体のプロパティです。 この警告メッセージは、パブリッシュ時にこのグラフの解像度を動的に制御できないことを示しています。 解決策としては、黄色い三角形が付いた出力のグラフプロパティに移動し、出力サイズの[継承方式](../../compositing-graphs/inheritance-compositing/inheritance-in-substance-compositing-graphs.md)を&#x200B;*親に相対的*&#x200B;に設定します。
