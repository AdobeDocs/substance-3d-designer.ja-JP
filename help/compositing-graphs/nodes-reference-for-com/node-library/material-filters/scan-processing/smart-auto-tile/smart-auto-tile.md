...
* **エッジ**
  ...
  * **しきい値**: *0.0 ～ 1.0*
  * **しきい値のBase color**: *0.0 - 1.0*
  * **しきい値標準**: *0.0 ～ 1.0*
  * **しきい値のHeight**: *0.0 - 1.0*
  * **切り取りのオフセット**: *0.0 - 0.5*&#x200B;切り取りを移動するためのメインコントロールです。X方向とY軸の両方が区切られています。
  * **ぼかし**: *0.0 ～ 2.0*&#x200B;描画トランジションをぼかします。
  * **Smoothness**: *0.0 ～ 2.0*&#x200B;エッジ分析結果のジャギーを制御します。
  * **グリッドの解像度**: *1 - 11*&#x200B;エッジ分析の解像度です。
  * **Base colorを使用**: *偽/真* Base colorの処理を切り替えます（インとアウト）。
