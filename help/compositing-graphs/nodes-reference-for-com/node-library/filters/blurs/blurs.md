---
helpx_url: "https://helpx.adobe.com/jp/substance-3d-designer/substance-compositing-graphs/nodes-reference-for-substance-compositing-graphs/node-library/filters/blurs.html"
breadcrumb-title: ''
description: Substance 3D Designerのぼかしフィルターノードを使用すると、テクスチャを滑らかにしたりソフトにしたりするための様々なブラーエフェクトを適用できます。
helpx_creative_field: ""
helpx_description: Designer > Substance compositing graphs > Nodes reference for Substance compositing graphs > Node library > Filters > Blurs
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: ブラー
user-guide-description: ''
user-guide-title: ''
source-git-commit: 6c55ac0f1f6da5bc5683a34a4eca174f978eac64
workflow-type: tm+mt
source-wordcount: '78'
ht-degree: 5%

---


# ブラー

[ぼかし]カテゴリが[標準のアトミックボックスぼかしノード](../../../../../compositing-graphs/nodes-reference-for-com/atomic-nodes/blur/blur.md)で展開されます。 より高度で高品質なぼかしは、複数の用途に使用できます。

ライブラリぼかしノードには、必ずグレースケールとカラーのバージョンがあります。 入力に適したバージョンを使用してください。

おすすめのブラー：

* [ブラー HQ](../../../../../compositing-graphs/nodes-reference-for-com/node-library/filters/blurs/blur-hq/blur-hq.md)
* [ぼかし(勾配)](../../../../../compositing-graphs/nodes-reference-for-com/node-library/filters/blurs/slope-blur/slope-blur.md)
