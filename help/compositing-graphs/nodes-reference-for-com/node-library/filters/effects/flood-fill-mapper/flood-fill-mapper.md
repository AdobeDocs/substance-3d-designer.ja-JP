...
* **タイリングモード**: *タイリングなし、H + V*&#x200B;タイリングを使用するかどうかを設定します。 サイズまたはスケールが1より小さく設定されている場合にのみ表示されます。
* **パターン**
...
* **サイズ**
  * **サイズモード**: *テクスチャを基準にする、図形の前面を基準にする、最大の図形を基準にする、最小の図形を基準にする、[図形の幅に合わせる]*&#x200B;各セルのパターンのサイズを決定する方法を設定します。
...
* **位置**
  ...
  * **オフセットのアラインメント**: *テクスチャ、パターン*&#x200B;オフセットの0ポイントをパターンセルまたはテクスチャに揃えるように設定します。
...
* **カラー** （グレースケールバージョンのみ）
  ...
  * **輝度のオフセット**: *-1.0 - 1.0*&#x200B;明るさコントロールとして機能する輝度のオフセットを設定します。
  * **輝度オフセットランダム**: *0.0 - 1.0*&#x200B;輝度オフセットのランダム化量を設定します。
  * **輝度オフセットマップマルチプライア**: *0.0 - 1.0*&#x200B;オプションの輝度オフセットマップの影響を設定します。
  * **背景色**: *（グレースケール値）*テクスチャを合成する背景色を設定します。
* **カラー** （カラーバージョンのみ）
  * **法線マップ**: *False/True*&#x200B;パターン入力を法線マップとして解釈するように設定します。 標準正接空間の回転を補正して修正します。
  * **標準の形式**: *DirectX、OpenGL*\
    法線マップ形式を切り替えます（グリーンチャンネルを反転します）。 Is 法線マップがTrueの場合にのみアクティブになります。
  * **HSL調整**: *-1.0 - 1.0* HSLをグローバルに調整します。
  * **HSLランダム**: *-1.0 - 1.0*&#x200B;セルごとにHSLのランダム化を設定します。
  * **Alpha調整**: *-1.0 ～ 1.0*&#x200B;全体的なAlpha調整を行い、Alphaのコントラストを下げます。
  * **Alphaのランダム**: *-1.0 - 1.0*&#x200B;セルごとにAlpha調整のランダム化を設定します。
  * **背景色**: *（カラー値）*テクスチャを合成する背景色を設定します。
