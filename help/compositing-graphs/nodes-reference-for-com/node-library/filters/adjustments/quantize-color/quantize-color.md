...
| <b>等高線のスムージング</b> *フロート* | 入力イメージに適用されるスムージングエフェクトの半径を制御します。このエフェクトは、量子化されたイメージをより立体的でまとまりのある形状に単純化するために使用されます。   メモ：このスムージングでは集中的な計算が必要なため、この値を大きくするとノードの計算時間が大幅に増加します。 |
| <b>ディザ処理</b> *フロート* | 元の画像のグラデーションとカラーブレンドを再現するためにディザリングパターンを適用します。量子化後に残ったカラーのみを使用します。   予想されるディザリング効果を生成するには、「等高線のスムージング」の値を0に設定してください。 |
...
| <b>フィルターをダウンスケール</b> *整数* | カラー量子化プロセスは、カラーを重要度別に分類するために、縮小（すなわち縮小）されたサイズの画像のヒストグラムを計算することを含む。 このパラメーターは、ヒストグラムを計算する前に、縮小された画像をフィルタリングする方法を制御します。<ul data-preserve-html="true"> <li data-preserve-html="true"><b>バイリニア：</b>では、画像にバイリニアフィルターが適用されます。その結果、元の画像には含まれない色が補間されたヒストグラムが作成され、元の色の一部が薄くなります。 これにより、多くのカラーを使用する画像で役立ちます。</li> <li data-preserve-html="true"><b>最も近いカラー：</b>フィルター処理なしで最も近いピクセルのカラーがサンプリングされ、元の画像のカラーのみを使用したヒストグラムが作成されます。 このオプションは、色数の少ない画像に適しています。</li> </ul> |
