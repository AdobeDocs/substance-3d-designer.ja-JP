...
|  |  |
| --- | --- |
| <b>出力</b> *色* | 変更したパレットの色をID マップのインデックスにマッピングした結果。 |
...
|  |  |
| --- | --- |
| <b>カラー選択モード</b> *整数* | 変更するパレット内のターゲットカラーを選択する方法。<ul data-preserve-html="true"> <li data-preserve-html="true"><b>カラーインデックス：</b>ターゲット色のインデックス</li> <li data-preserve-html="true"><b>イメージスペース：</b>インデックスをサンプリングするID マップ内の位置です。 このモードを選択すると、2D ビュー内で位置ギズモを使用して簡単に選択できるようになります</li> </ul> |
| <b>カラー位置</b> *浮動小数2* *&#39;カラー選択モード&#39;が&#39;イメージ領域&#39;に設定されている場合に使用可能* | ID マップ内で索引を抽出する位置を指定します。   2D ビュー内のギズモを使用すると、画像内の場所を簡単に選択できます。   ヒント：ギズモの抽出元の量子化されたイメージを表示し、[カラーID マップを修正]ノードを選択してギズモを表示することができます。 これにより、カラーを選択してより直感的に変更できるようになります。 |
| <b>カラーインデックス</b> *整数* *&#39;カラー選択モード&#39;が&#39;カラーインデックス&#39;に設定されている場合に使用可能* | ターゲットカラーのインデックス。   パレットの色は左から右に並べられ、最初の色のインデックスは0です。 |
| <b>カラー選択スプレッド</b> *浮動小数* | 選択範囲が隣接するカラーに達する範囲を制御します。   色は、*立方体*&#x200B;に配置され、幅、Height、および深度は、色の各構成要素が0から1に増加するグラデーションです(例： 赤、緑、青(RGB)。   このパラメーターは、立方体の選択したカラーの周囲にある他のカラーも変更可能な範囲を調整します。1は立方体の幅の全体です。 |
