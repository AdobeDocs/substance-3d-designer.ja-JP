---
helpx_url: "https://helpx.adobe.com/jp/substance-3d-designer/substance-compositing-graphs/nodes-reference-for-substance-compositing-graphs/node-library/filters/adjustments/hdr-range-viewer.html"
breadcrumb-title: ''
description: 「HDR範囲ビューア」ノードを使用すると、HDRのテクスチャ範囲を視覚化して、ハイダイナミックレンジ値の分析や調整を行うことができます。
helpx_creative_field: ""
helpx_description: Designer > Substance compositing graphs > Nodes reference for Substance compositing graphs > Node library > Filters > Adjustments > HDR Range Viewer
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: HDR範囲ビューア
user-guide-description: ''
user-guide-title: ''
source-git-commit: 029f702d9b6a4d0dfaa83a4ae8447c02f70be355
workflow-type: tm+mt
source-wordcount: '66'
ht-degree: 10%

---


# HDR範囲ビューア

<table>
<tr style="border: 0;">
<td width="33.33%" style="border: 0;" valign="top">

![](../../../../../../assets/hdr-range-viewer-grayscale.png){width="128px"}

![](../../../../../../assets/hdr-range-viewer.png){width="128px"}

<b>イン:</b>フィルター/調整

</td>
<td width="100.00%" style="border: 0;" valign="top">

## 説明

ハイダイナミックレンジで正確な領域を確認するためのデバッグツール。 カラーバージョンとグレースケールバージョンの両方が存在します。

</td>
</tr>
</table>

<a name="parameters"></a>

## パラメーター

|  |  |
|:---|:---|
| <b>範囲の最小値</b> <i>-2.0 - 0.0</i> | ハイライト表示を開始する最小範囲。 |
| <b>範囲の最大値</b> <i>1.0 - 3.0</i> | ハイライト表示する最大の範囲。 |

## 例

<table style="margin-top: 32px; margin-bottom: 32px">
    <tr style="border: 0">
        <td style="border: 0; background: transparent">
            <img src="../../../../../../assets/hdr-viewer-example.png" />
        </td>
    </tr>
</table>
