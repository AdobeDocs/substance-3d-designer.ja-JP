...
* **プロファイルの種類**: *直線、垂直グラデーション、マスク*&#x200B;効果の動作を決定し、オプションの追加入力マップを使用するためのメインコントロール。\
  「直線」は標準の「押し出し」動作、「垂直グラデーション」は軸全体に沿ったカスタムスケール値、「マスク」はマスクごとに軸に沿ったセクションを非表示にできます。
* **ベベルHeight**: *0.0 ～ 1.0*&#x200B;押し出しの軸に沿ってベベルが到達する距離を設定します。
...
* **位置**:\
  3D空間で結果を回転するためのメインコントロール。 2Dビューのインターエイブティスギズモと相関します。
