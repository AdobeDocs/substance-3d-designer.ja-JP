---
helpx_url: "https://helpx.adobe.com/substance-3d-designer/substance-compositing-graphs/nodes-reference-for-substance-compositing-graphs/node-library/texture-generators/noises/grunge-galvanic-small.html"
breadcrumb-title: ""
description: 経年劣化ガルバニック小ノードを使用して、金属風化効果のための小規模なガルバニック腐食パターンを生成します。
helpx_creative_field: ""
helpx_description: Designer > Substance compositing graphs > Nodes reference for Substance compositing graphs > Node library > Texture Generators > Noises > Grunge Galvanic Small
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: ガルバニック経年劣化 （小）
user-guide-description: ""
user-guide-title: ""
source-git-commit: 0f214099ae94088d37122a5d474d3e70d4ccf46f
workflow-type: tm+mt
source-wordcount: '133'
ht-degree: 2%
---

# ガルバニック経年劣化 （小）

<table>
<tr style="border: 0;">
<td width="33.33%" style="border: 0;" valign="top">

![](grunge-galvanic-small.resources/grungegalvanicsmall.jpg){width="200px"}

<b>In:</b>テクスチャジェネレーター>ノイズ

</td>
<td width="100.00%" style="border: 0;" valign="top">

## 説明

**経年劣化ガルバニックスモール**&#x200B;ノードは、高密度の亜鉛メッキ鋼のパターンに似た経年劣化マップを生成します。

</td>
</tr>
</table>

<a name="parameters"></a>

## パラメーター

|  |  |
|:---|:---|
| <b>残高</b> <i>フロート</i> | 暗い値と明るい値のバランスを調整します。 |
| <b>コントラスト</b> <i>フロート</i> | 画像のコントラストを調整します。 |
| <b>反転</b> <i>ブール値</i> | `1-x`操作を使用して画像の出力を反転します。 |
| <b>非正方形拡張</b> <i>ブール値</i> | スカッシュとストレッチを非正方形の比率で補正できます。 |
| <b>詳細</b> |  |
| <b>鮮明さ</b> <i>フロート</i> | 亜鉛メッキを施したシェイプの鮮明さとシャープさを調整します。 |
| <b>Dirt</b> <i>フロート</i> | Dirtオーバーレイの不透明度を調整します。 |
| <b>マイクロゆがみ</b> <i>フロート</i> | 高周波ワープ効果の強度を調整します。 |

## 例

<table style="table-layout:fixed">
    <tr style="border: 0;">
        <td style="border: 0;">
            <img src="grunge-galvanic-small.resources/grungegalvanicsmall-variant2.jpg" class="modal-image" alt="ガルバニック経年劣化小型 – 例1" />
        </td>
        <td style="border: 0;">
            <img src="grunge-galvanic-small.resources/grungegalvanicsmall-variant.jpg" class="modal-image" alt="ガルバニック経年劣化小型 – 例2" />
        </td>
        <td style="border: 0;"></td>
    </tr>
</table>
