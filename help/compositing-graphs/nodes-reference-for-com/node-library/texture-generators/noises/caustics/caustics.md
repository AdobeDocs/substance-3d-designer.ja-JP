...
Heightマップとライトの方向に基づいて投影されたコースティクスを生成します。グレースケールとカラーの両方のバージョンがありますが、違いは微妙ですが、カラーバージョンでは色分散効果が追加されます。 光は1つの点から照射され、環境マップは使用されません。
...
* **フォトングリッドサイズ**: *自動、512、1024、2048、4096*\
  グリッドサイズを調整して品質を設定しますが、デフォルトでは入力が一致します。 計算の高速化に使用できます。
...
* **分散**: *0.0 ～ 0.01 （カラーバージョンのみ）*\
  色の分散のみに影響します。 IORが低い場合は表示されません。
* **ジッター**: *0.0 ～ 1.0*\
  不規則なジッターをキャストフォトンパーティクルに追加します。
* **明るい位置**:\
  ライトの位置を移動します。 2Dビューのギズモを使用しても同様です。
* **背景色**: *（カラー値） （カラーバージョンのみ）*\
