...
* **位置の座標**
  ...
  * **ポイント1のUV位置**:\
    地面/天井と原点のみ。 UV空間の最初のポイント位置を設定します。
  * **ポイント2 UVの位置**:\
...
* **行の回転**: *0.0 ～ 1.0*\
  線分の長さに沿って線分を軸します。 ラインは回転時にフラットカードとして扱われます。
* **回線Thickness**: *0.0 - 1.0*\
...
* **パターン繰り返し間隔**: *0.0 ～ 1.0*\
  パターンUVモードが「繰り返し+間隔」に設定されている場合のみ。 繰り返されるパターン間の間隔の量を設定します。
* **グラウンドクリッピングを有効にする**: *False/True*\
  線画のクリッピングを有効にします。 地面/天井の配置モードを使用している場合、効果は表示されません。
* **地上Height**: *-2.0 - 0.0*\
  クリッピングに使用するフロンド平面の相対Heightを設定します。 描画された地盤グリッドに影響します。
