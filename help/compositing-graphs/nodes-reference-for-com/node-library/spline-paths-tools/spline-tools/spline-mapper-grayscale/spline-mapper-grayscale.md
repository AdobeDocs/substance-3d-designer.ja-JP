...
![ノードアイコン](spline-mapper-grayscale.resources/spline-mapper-grayscale-icon.png "ノードアイコン")
...
入力スプラインに沿って伸びるプリミティブシェイプに入力グレースケールイメージをマップします。
...
マップされた各スプラインを個別に選択するためのHeight、UV（イメージ座標）、IDマスクなどの他の情報と同様に、マップされたイメージをグレースケールイメージとして出力します。
...
>[!NOTE]
>
> [スプラインマッパーの色](../../../../../../compositing-graphs/nodes-reference-for-com/node-library/spline-paths-tools/spline-tools/spline-mapper-color/spline-mapper-color.md)も参照してください。
...
| <b>カラーマップ</b> <i>グレースケール</i> | 入力スプラインに沿ってマップされる入力グレースケールイメージ。 |
| <b>Heightマップ</b> <i>グレースケール</i> | 入力スプラインに沿ってマップされる入力グレースケール高さマップ。 |
...
|  |  |
|:---|:---|
| <b>色</b> <i>グレースケール</i> | 入力スプラインに入力カラー画像をマッピングした結果をグレースケールイメージします。 |
...
| <b>シリンダHeight乗数</b> <i>浮動小数</i> | （[形状]が[半円柱]または[円柱]に設定されている場合に使用可能）Height出力における円柱のHeightの割合の強度の乗数。<br>Height調整は累積的です。 |
...
| <b>UVのツイスト強度</b> <i>浮動小数</i> | （「シェイプ」が「半円柱」または「円柱」に設定されている場合に使用可能）円柱の周囲のイメージのツイストです。<br>ねじれにより、スプラインの端のみで円柱が回転します。 次に、回転がスプラインに沿って補間されます。 |
...
| <b>ツイストUVカーブオフセット</b> <i>浮動小数</i> | （&#39;Shape&#39;が&#39;Half Cylinder&#39;または&#39;Cylinder&#39;の場合に使用可能）Twist Curveによって指定された回転値に対して、回転の回数でグローバルオフセットを適用します。 |
| <b>スプラインHeight乗数</b> <i>浮動小数</i> | Height出力に対するスプラインHeight入力の影響の強さを調整します。<br>Height調整は累積的です。 |
| <b>入力Height乗数</b> <i>浮動小数</i> | Height出力に対する高さマップ入力の影響の強さを調整します。<br>Height調整は累積的です。 |
...
![ノードの例2](spline-mapper-grayscale.resources/SplineMapperGrayscale-Demo.gif "ノードの例2")
...
![ノードの例3](spline-mapper-grayscale.resources/SplineMapperGrayscale-Variant1-After1.jpg "ノードの例3")
