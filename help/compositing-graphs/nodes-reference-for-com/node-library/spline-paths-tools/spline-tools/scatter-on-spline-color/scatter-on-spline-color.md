...
|  |  |
|:---|:---|
| <b>スプライン入力</b> <i>整数</i> | スキャッタリングパターンに使用するスプラインの選択方法：<br>**&#x200B;すべてのスプライン&#x200B;*：入力リストのすべてのスプラインを使用します。<br>* *単一スプライン*：入力リストから指定されたスプラインのみを使用します。<br>* *スプライン範囲*：入力リストから指定された範囲のスプラインのみを使用します。 |
...
| <b>ランダムな色の中心</b> <i>浮動小数</i> | [ランダムカラー]に適用されたランダムオフセットの範囲にオフセットを適用します<br>値–1は、すべてのランダム値が高いことを意味し、値1は、すべてのランダム値が低いことを意味します。 |
| <b>スプラインThickness乗数</b> <i>浮動小数</i> | 各パターンの色が、その場所でのスプラインのThicknessに対して乗算される強さ。<br>注意：出力カラーは、すべてのカラー乗数の重み付けされた結果です。 |
| <b>シェイプのスケール乗数</b> <i>浮動小数</i> | 各パターンの色がそのスケールに対して乗算される強度です。<br>注意：出力される色は、すべての色の乗数の重み付けされた結果です。 |
| <b>図形インデックス乗数</b> <i>浮動小数</i> | 各パターンの色が正規化されたインデックスに対して乗算される強度です。<br>注意：出力される色は、すべての色の乗数の重み付けされた結果です。 |
| <b>スプラインHeight乗数</b> <i>浮動小数</i> | 各パターンの色が、その場所でのスプラインのHeightに対して乗算される強さ。<br>注意：出力カラーは、すべてのカラー乗数の重み付けされた結果です。 |
| <b>ランダム輝度</b> <i>浮動小数</i> | 指定された値までランダム乗数を適用して、パターンの輝度を下げます。<br>注意：出力カラーは、すべてのカラー乗数の重み付けされた結果です。 |
| <b>スプラインThickness乗数</b> <i>浮動小数</i> | 各パターンのアルファがスプラインのThicknessに対して、その場所で乗算される強さ。<br>注：出力カラーは、すべてのカラー乗数の重み付けされた結果です。 |
| <b>シェイプのスケール乗数</b> <i>浮動小数</i> | 各パターンのアルファをそのスケールに対して乗算する強度です。<br>注意：出力カラーはすべてのカラー乗数の重み付けされた結果です。 |
...
| <b>マスクマップを反転</b> <i>ブーリアン</i> | 「1 - x」のマイナス演算を使用して、マスクマップの値を反転します。 |
| <b>マスクを反転</b> <i>ブーリアン</i> | パターンのマスクを反転します。 |
| <b>非正方形の修正</b> <i>ブーリアン</i> | 非正方形の解像度でスプラインのシェイプを保持するために、ポイントの位置を調整します。 |
