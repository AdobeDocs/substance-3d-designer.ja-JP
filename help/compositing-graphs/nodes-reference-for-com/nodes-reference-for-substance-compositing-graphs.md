---
helpx_url: "https://helpx.adobe.com/jp/substance-3d-designer/substance-compositing-graphs/nodes-reference-for-substance-compositing-graphs.html"
breadcrumb-title: ''
description: 使用可能なすべてのノードについて詳しくは、DesignerのSubstance合成グラフの完全なノードリファレンスを参照してください。
helpx_creative_field: ""
helpx_description: Designer > Substance graphs > Nodes reference for Substance graphs
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: Substance グラフのノード参照
user-guide-description: ''
user-guide-title: ''
source-git-commit: 4f8830fa9ab6012f0a7ba5054eb171b151c44874
workflow-type: tm+mt
source-wordcount: '121'
ht-degree: 16%

---


# Substance グラフのノード参照

Substance 3D Designerはノードベースのテクスチャリングソフトウェアであるため、さまざまなノードを自由に使用できます。 このセクションでは、さまざまなタイプについて説明し、ソフトウェアに付属しているすべてのノードについて説明します。

SD内の「ノード」と呼ばれるものは、2つのメイングループに分けられます。

* [アトミックノード](../../compositing-graphs/nodes-reference-for-com/atomic-nodes/atomic-nodes.md)。あらゆるグラフの主要な構成要素です。
* [グラフインスタンス](../../compositing-graphs/creating-compositing-gra/graph-instances-sub-gra/graph-instances-sub-graphs.md)、複合ノードのうち、事前に作成されたコンテンツの大きなセレクションが[ノードライブラリ](../../compositing-graphs/nodes-reference-for-com/node-library/node-library.md)で利用できます

## このセクションの内容：

<table>
<tr style="border: 0;">
<td style="border: 0;" valign="top">

[ノードライブラリ](../../compositing-graphs/nodes-reference-for-com/node-library/node-library.md)

* [テクスチャ生成](node-library/texture-generators/texture-generators.md)
* [フィルター](node-library/filters/filters.md)
* [マテリアルフィルター](node-library/material-filters/material-filters.md)
* [メッシュベースの生成](node-library/mesh-based-generators/mesh-based-generators.md)
* [スプラインとパスツール](node-library/spline-paths-tools/spline-paths-tools.md)

</td>
<td style="border: 0;" valign="top">

[アトミックノード](../../compositing-graphs/nodes-reference-for-com/atomic-nodes/atomic-nodes.md)

</td>
</tr>
</table>
