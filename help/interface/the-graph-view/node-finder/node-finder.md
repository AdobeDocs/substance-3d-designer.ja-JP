...
* インスタンスノードによって参照されるグラフの<b>ID</b>
* ノードパラメーター関数で使用される、公開されたパラメーターまたは変数の<b>識別子</b>
* ノードの<b>UID</b> （一意の識別子）
* ノードの<b>ラベル</b>
...
グラフビューのツールバーで、<b>ノードファインダーボタン![](../../../assets/graph-node-finder.png)</b>をクリックして、ノードファインダーツールバーを表示します。 表示されたツールバーは、このボタンをクリックすることによってのみ閉じられます。

<b>トラバースグラフを検索します</b>。 つまり、次のアクションを使用してグラフを開くと、検索がアクティブなままになります。

* インスタンスノード：コンテキスト内の参照を開く(Ctrl+E / Cmd+E) （*注：*&#x200B;コンテキスト内のグラフ編集は、編集/環境設定/グラフで有効にする必要があります）
* ピクセルプロセッサー：編集機能(Ctrl+E/Cmd+E)
* 値プロセッサ：編集機能(Ctrl+E/Cmd+E)
