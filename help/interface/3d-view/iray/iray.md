...
> 
> 詳細については、こちらを参照してください： [MDL グラフとIrayの提供が終了しました](../../../technical-issues/mdl-graph-iray-eol/mdl-graph-iray-eol.md)
...
<b>Iray</b>は、光とマテリアルの物理的な動きをシミュレートすることで&#x200B;*フォトリアルな画像*&#x200B;を生成する、高度に&#x200B;*インタラクティブ*&#x200B;で直感的な物理ベースのレンダリングテクノロジーです。 詳しくは、[NVIDIA Iray](https://www.nvidia.com/en-us/design-visualization/iray/)のwebページを参照してください。
...
3D ビューはIrayの&#x200B;*プログレッシブレンダラー*&#x200B;を使用するため、各ピクセルに少なくとも1つのサンプルが実行されると、すぐにイメージが生成されます。 サンプリング反復の実行中に画像が&#x200B;*自動更新*&#x200B;されます。その結果、最初の大まかな画像が各反復ーで&#x200B;*より鮮明になります*。

レンダラーは[3Dビュー](../../../interface/3d-view/3d-view.md)パネルで使用できます。<b>レンダラー</b>メニューを開き、<b>Iray</b>オプションをクリックして、その3Dビューパネルで使用されているレンダラーをIrayに切り替えます。\
Irayレンダラー&#x200B;*に切り替えると、一部の3Dビューメニューで使用できるオプション*&#x200B;が変わります。 これらの変更については、以下の「<b>3Dビュー</b>」セクションで説明しています。

デフォルトでは、Irayレンダラを選択するとすぐにプログレッシブレンダリングが開始されます。 レンダリングプロセスは、次の条件のうち&#x200B;*one*&#x200B;が満たされるまで実行されます：
