...
<b>SDApplication</b>クラスからアクセスできる<b> SDSpotColorLibrary </b>クラスには、Designerに含まれるスポットカラーライブラリに関する情報が含まれています。

このクラスを使用すると、カラーブックとスポットカラーを一覧表示し、特定のスポットカラーまたは特定のRGBカラーに最も近いスポットカラーを検索できます。

<b>OpenColorIO</b>を使用している場合、Designerでスポットカラーを&#x200B;*利用できません*。 この場合、app.getSpotColorLibrary()は<b>None</b>を返します。
...
スポットカラーは、ノードプロパティから取得したり、ノードプロパティに設定したりできます。
