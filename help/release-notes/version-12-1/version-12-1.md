...
* [UI] エクスプローラーパネルのタイトルバーから「ピン」ボタンを削除する
* [UI]複数のバージョンでようこそ画面の「次回から表示しない」オプションを保存する
* [3D ビュー] 「軸」ヘルパーが有効になっている場合、ビューポートにグリッドユニットを表示します
* [自動処理] Designerでsbsbakerコマンドラインツールを使用する
* [カラーマネジメント] Adobe ACE用に新しいGPUバックエンドを実装
* [Cooker]タイムスタンプなしでパッケージを調理するオプションを追加します
* [グラフ] FxMap グラフにバッジを追加します。
...
* [MDL]入力が未接続のドットノードに接続されている場合、表示されるパラメーターは0に初期化されます
